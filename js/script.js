...
$(function () {
    let imgHeight = $('.p-hero__wrapper').outerHeight();  //キービジュアルの画像の高さを取得
    if (!imgHeight) imgHeight = $('.p-hero_top__wrapper').outerHeight();
    let bgHeight = $('.p-hero').outerHeight(); //ファーストビューの高さを取得
    if (!bgHeight) bgHeight = $('.p-hero_top').outerHeight();
